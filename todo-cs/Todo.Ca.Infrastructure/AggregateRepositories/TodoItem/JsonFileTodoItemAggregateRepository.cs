...
namespace Todo.Ca.Infrastructure.AggregateRepositories.TodoItem;
...
public class JsonFileTodoItemAggregateRepository : ITodoItemAggregateRepository
{
    ...
    public JsonFileTodoItemAggregateRepository()
    {
        ...
        if (string.IsNullOrEmpty(dataHome))
        {
            ...
            dataHome = Path.Combine(dataHome, "todo");
        }
    ...
    private List<TodoItemAggregate>? ReadData()
    {
        ...
        var todoItemDtoList = JsonSerializer.Deserialize<List<TodoItemDto>>(jsonData, jsonSerializerOptions);
    ...
    private void WriteData(List<TodoItemAggregate> items)
    {
        ...
        var jsonData = JsonSerializer.Serialize(todoItemDtoList, jsonSerializerOptions);
        File.WriteAllText(_filePath, jsonData);
    }
    
    private record TodoItemDto(string Id, string Name,  bool IsComplete);
}
